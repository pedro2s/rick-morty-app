...
import type { Character, CharactersResponse, CharacterResponse, CharacterFilter, Info } from '@/types/rick-morty'
...
export function useCharacters(page: Ref<number>, filter: Ref<CharacterFilter>) {
  ...
  const info = computed<Info>(() => result.value?.characters.info ?? { count: 0, pages: 0, next: null, prev: null });
